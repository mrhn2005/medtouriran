...
function elono_like_scripts() {
	if( !defined('LARUS_PATH') ) { define('LARUS_PATH', ( plugins_url() . '/elono-functions') ); } 
	wp_enqueue_script( 'elono_like_post',  plugin_dir_url( __FILE__ ) . 'post-like.js', array('jquery'), '1.0', 1 );
	wp_localize_script( 'elono_like_post', 'ajax_var', array(
		'url' => admin_url( 'admin-ajax.php' ),
		'nonce' => wp_create_nonce( 'ajax-nonce' )
		)
	);
}
add_action( 'init', 'elono_like_scripts' );

/**
 * (3) Save like data
 */
add_action( 'wp_ajax_nopriv_jm-post-like', 'elono_post_like' );
add_action( 'wp_ajax_jm-post-like', 'elono_post_like' );
function elono_post_like() {
	$nonce = $_POST['nonce'];
    if ( ! wp_verify_nonce( $nonce, 'ajax-nonce' ) )
        die ( 'Nope!' );
	
	if ( isset( $_POST['elono_post_like'] ) ) {
 ...
 * (5) Front end button
 */
function elono_getPostLikeLink( $post_id ) {
 ...
 */
add_action( 'show_user_profile', 'elono_show_user_likes' );
add_action( 'edit_user_profile', 'elono_show_user_likes' );
function elono_show_user_likes( $user ) { ?>        
 ...
 */
function elono_frontEndUserLikes() {
	if ( is_user_logged_in() ) { // user is logged in
		$like_list = '';
		$user_id = get_current_user_id(); // current user
 ...
 * Markup assumes sidebar/widget usage
 */
function elono_most_popular_today() {
	global $post;
	$today = date('j');
  	$year = date('Y');
	$args = array(
 ...
 * Markup assumes sidebar/widget usage
 */
function elono_most_popular_month() {
	global $post;
	$month = date('m');
  	$year = date('Y');
	$args = array(
 ...
 * Markup assumes sidebar/widget usage
 */
function elono_most_popular_week() {
	global $post;
	$week = date('W');
  	$year = date('Y');
	$args = array(
